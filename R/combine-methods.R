...
setMethod("combineRows", "SElite", function(x, ..., delayed=TRUE, fill=NA, use.names=TRUE) {
    all.se <- list(x, ...)
    ...
    tryCatch({
        ...
        stop(paste0("failed to combine rowData of SElite objects:\n  ", conditionMessage(e)))
    })
    ...
    tryCatch({
        ...
        stop(paste0("failed to combine colData of SElite objects:\n  ", conditionMessage(e)))
    })
    ...
    # Finally, filling in the rowRanges. Rows for SElite
...
combine_assays_by <- function(all.se, mappings, delayed, fill, by.row) {
    ...
    if (any(no.assay.names)) {
        ...
        if (length(n.assays)!=1L) {
            stop("all SElites should have the same number of unnamed assays")
        }
...
extract_granges_from_se <- function(all.se) {
    has.ranges <- vapply(all.se, is, class2="RangedSElite", FUN.VALUE=TRUE)
    if (!any(has.ranges)) {
        return(NULL)
    }
...
setMethod("combineCols", "SElite", function(x, ..., delayed=TRUE, fill=NA, use.names=TRUE) {
    all.se <- list(x, ...)
    ...
    if (!is.null(mappings)) {
        ...
        for (i in seq_along(extracted.ranges)) {
            ...
            if (!identical(new.rr[existing], old.rr[existing])) {
                warning(wmsg("different 'rowRanges' for shared rows across SElite objects, ",
