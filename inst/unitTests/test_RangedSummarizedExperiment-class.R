...
test_RangedSElite_construction <- function()
{
    ## empty-ish
    m1 <- matrix(0, 0, 0)
    checkTrue(validObject(new("RangedSElite")))
...
test_RangedSElite_getters <- function()
{
    for (i in seq_along(rseList)) {
        rse <- rseList[[i]]
...
test_RangedSElite_setters <- function()
{
    for (i in seq_along(rseList)) {
        rse <- rseList[[i]]
        rowRanges <- rowRangesList[[i]]
...
test_RangedSElite_subset <- function()
{
    for (i in seq_along(rseList)) {
        rse <- rseList[[i]]
        rowRanges <- rowRangesList[[i]]
    ...
    ## 0 columns
    se <- SElite(rowRanges=GRanges("chr1", IRanges(1:10, width=1)))
...
test_RangedSElite_subsetassign <- function()
{
    for (i in seq_along(rseList)) {
        rse <- rseList[[i]]
...
test_RangedSElite_cbind <- function()
...
test_RangedSElite_rbind <- function()
...
test_RangedSElite_GRanges_values <- function()
{
    x <- rseList[[1]]
...
test_RangedSElite_split <- function()
{
    gr <- GRanges(Rle(c("A", "B"), c(2, 3)), IRanges(1:5, 10))
    se <- SElite(M1, rowRanges=gr, colData=colData)
...
test_RangedSElite_NULL_rowRanges <- function()
{
    se <- SElite(M1, colData=colData)
    rse <- rseList[[1L]]
    rowRanges(rse) <- NULL
    checkTrue(identical(rowRanges(rse), NULL))
    checkTrue(is(rse, "SElite") &&
        !is(rse, "RangedSElite"))
