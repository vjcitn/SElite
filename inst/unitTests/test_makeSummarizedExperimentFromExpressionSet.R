...
test_SElite_GenomicRanges_coercion <- function()
{
        eset1 <- ExpressionSet()

        checkTrue(validObject(eset1))

        se1 <- as(eset1, "RangedSElite")
...
test_GenomicRanges_SElite_coercion <- function()
{
        ## empty SE
        simpleSE <- SElite()
        ...
        simpleES2 <- as(as(simpleES, "RangedSElite"),
                        "ExpressionSet")
        ...
        ## SElite rows if they are from GRanges.
        ...
        ## SElite rows if they are from GRangesList, but they
        ...
        eset5 <- as(as(eset4, "RangedSElite"), "ExpressionSet")
...
test_GenomicRanges_SElite_coercion_lockedEnvironment <- function()
{
    ## https://github.com/Bioconductor/SElite/issues/43
    se = SElite(list(exprs = matrix(1:10, 5)))
...
test_GenomicRanges_SElite_coercion_mappingFunctions <- function()
{
    ## naiveRangeMapper
    ## valid object from empty object
    checkTrue(validObject(makeSEliteFromExpressionSet(ExpressionSet())))

    ## valid object from sample ExpressionSet
    data("sample.ExpressionSet", package = "Biobase")
    eset1 <- sample.ExpressionSet
    checkTrue(validObject(makeSEliteFromExpressionSet(eset1)))

    ## makeSEliteFromExpressionSet should be the same as `as`
    ## with default args
    checkEquals(makeSEliteFromExpressionSet(eset1),
                as(eset1, "RangedSElite"))

    ## probeRangeMapper
    ## valid object from empty object
    checkTrue(validObject(
            makeSEliteFromExpressionSet(ExpressionSet(),
                probeRangeMapper)))

    ## valid object from sample ExpressionSet
    se1 <- makeSEliteFromExpressionSet(eset1, probeRangeMapper)
    checkTrue(validObject(se1))
    ...
    ## valid object from sample ExpressionSet
    se2 <- makeSEliteFromExpressionSet(eset1,
