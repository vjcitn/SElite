...
test_SElite_construction <- function()
{
    ## empty-ish
    m1 <- matrix(0, 0, 0)
    checkTrue(validObject(new("SElite")))
    checkTrue(validObject(SElite()), "empty constructor")
    checkTrue(validObject(SElite(SimpleList())))
    checkTrue(validObject(SElite(assays=SimpleList(m1))),
              "0x0 constructor")
    checkException(SElite(assays=SimpleList(m1, matrix())),
    ...
    checkTrue(validObject(se <- SElite(m)))
    ...
    checkTrue(validObject(SElite(list(df))))
}

test_SElite_construction_dimnames <- function()
{
    do_tests <- function(m, rowData, colData) {
        ...
        se <- SElite(m, rowData=rowData)
        checkTrue(validObject(se))
        checkIdentical(target_rownames(), rownames(se))
        checkIdentical(colnames(m), colnames(se))
        test_rowData_colData_assay(se)

        se <- SElite(m, colData=colData)
        checkTrue(validObject(se))
        checkIdentical(rownames(m), rownames(se))
        checkIdentical(target_colnames(), colnames(se))
        test_rowData_colData_assay(se)

        se <- SElite(m, rowData=rowData, colData=colData)
        checkTrue(validObject(se))
        checkIdentical(target_rownames(), rownames(se))
        checkIdentical(target_colnames(), colnames(se))
        test_rowData_colData_assay(se)
    ...
    ##   Right now they break the SElite() constructor in an ugly
    ...
    ## SElite() constructor were to take care of the fix, it
...
test_SElite_getters <- function()
{
    ...
    checkIdentical(a, assays(SElite(assays=a)))
    ## assay
    checkException(
        assay(SElite()), "0-length assay", TRUE)
    checkIdentical(m0,
        assay(SElite(assays=a)), "default assay")
    checkIdentical(m1,
        assay(SElite(assays=a), 2),
        "assay, numeric index")
    checkException(
        assay(SElite(assays=a), 3),
        "invalid assay index", TRUE)
    checkIdentical(m1,
        assay(SElite(assays=a), "b"),
        "assay, character index")
    checkException(
        assay(SElite(assays=a), "c"),
        "invalid assay name", TRUE)
}

test_SElite_setters <- function()
{
    for (i in seq_along(se0List)) {
        se0 <- se0List[[i]]
...
test_SElite_subset <- function()
{
    ...
    ## 0 columns
    se <- SElite(matrix(integer(0), nrow=5))
    checkIdentical(dim(se[1:5, ]), c(5L, 0L))
    ## 0 rows
    se <- SElite(colData=DataFrame(samples=1:10))
    checkIdentical(dim(se[ ,1:5]), c(0L, 5L))
}

test_SElite_subsetassign <- function()
{
    for (i in seq_along(se0List)) {
        se0 <- se0List[[i]]
...
test_SElite_assays_4d <- function()
{
    ...
    checkTrue(validObject(SElite(assays0)))

    dimnames(B)[[2]] <- c("y1", "oops")
    assays0 <- SimpleList(A=A, B=B)
    checkException(SElite(assays0))
