test_combineRows_unnamed <- function() {
    se <- SElite(list(counts=matrix(rpois(1000, 10), ncol=10)))
    colData(se)$A <- 1
    rowData(se)$A <- 1

    se2 <- SElite(list(counts=matrix(rpois(1000, 10), ncol=10), 
...
test_combineRows_named <- function() {
    ...
    se2 <- SElite(list(counts=matrix(rpois(1000, 10), ncol=20), 
...
test_combineRows_assays <- function() {
    ...
    se <- SElite(list(matrix(rpois(1000, 10), ncol=10)))
    se2 <- SElite(list(matrix(rpois(1000, 10), ncol=10), 
...
test_combineRows_ranges_named <- function() {
    ...
    checkIdentical(as.character(class(out)), "SElite")
    checkIdentical(rownames(out), c(rownames(se), rownames(se2)))
...
test_combineCols_named <- function() {
    se <- SElite(list(counts=matrix(rpois(1000, 10), ncol=100)))
    colData(se)$A <- 1L
    rowData(se)$A <- 1

    se2 <- SElite(list(counts=matrix(rpois(1000, 10), ncol=50), 
...
test_combineCols_ranges_named <- function() {
    ...
    checkIdentical(as.character(class(out)), "RangedSElite")
